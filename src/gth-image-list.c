			  ...
			  GdkEventKey *event)
{
	...
	if ((image_list->priv->selection_mode == GTK_SELECTION_MULTIPLE)
	    && ! image_list->priv->multi_selecting_with_keyboard
			     ...
			     GdkEventButton *event)
{
	...
	if ((pos != -1) 
		...
		if (priv->selection_mode == GTK_SELECTION_MULTIPLE)
			...
			selection_single_button_press (image_list, event, pos);
	}
